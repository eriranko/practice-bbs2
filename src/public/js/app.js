...
function likePost(postId) {
  ...
  fetch("/posts/".concat(postId, "/like"), {
    ...
    if (data.success) {
      var likesCountElement = document.getElementById("likes-count-".concat(postId));
    ...
    alert("Error: ".concat(error.message)); // エラーメッセージをアラートで表示
  });
}
...
function agreePost(postId) {
  ...
  fetch("/posts/".concat(postId, "/agree"), {
    ...
    if (data.success) {
      var agreeCountElement = document.getElementById("agree-count-".concat(postId));
      if (agreeCountElement) {
        agreeCountElement.textContent = (parseInt(agreeCountElement.textContent) + 1).toString();
      } else {
...
function deletePost(postId) {
  if (confirm("この投稿を削除しますか？")) {
    fetch("/posts/".concat(postId), {
      ...
      if (data.success) {
        // 投稿をDOMから削除
        var postElement = document.querySelector(".post[data-id=\"".concat(postId, "\"]"));
        if (postElement) {
          postElement.remove();
        }
      } else {
...
function toggleReplies(commentId, button) {
  var repliesWrapper = document.getElementById("replies-".concat(commentId));
  if (repliesWrapper) {
    if (repliesWrapper.style.display === "none" || repliesWrapper.style.display === "") {
      ...
      button.textContent = "\u30B3\u30E1\u30F3\u30C8\u3078\u306E\u8FD4\u4FE1\uFF08".concat(repliesWrapper.children.length, "\uFF09\u3092\u8868\u793A"); // ボタンのテキストを元に戻す
    }
  }
}
...
function toggleComments(postId) {
  var commentsWrapper = document.getElementById("all-comments-".concat(postId));
